...
// Seek, SeekToFirst, SeekToLast, Error, Valid, and Close will still be
// safe to call.
//
// method Error() will only return an error in the event of a LevelDB error. It will
// return a nil on iterators that are simply invalid. Given that behavior,
// Error is not a replacement for a Valid.
//
...
func (it *Iterator) Seek(key []byte) {
	...
	C.leveldb_iter_seek(it.iter, keyPtr, C.size_t(keyLen))
}

// Error returns an IteratorError from LevelDB if it had one during
// iteration.
//
// This method is safe to call when Valid returns false.
